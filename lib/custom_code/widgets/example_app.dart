...
class _ExampleAppState extends State<ExampleApp> {
  final _streamController = StreamController<geofence.PolyGeofence>();

  // Create a [PolyGeofenceService] instance and set options.
  final _polyGeofenceService = geofence.PolyGeofenceService.instance.setup(
      interval: 1000,
      accuracy: 100,
  ...
  final _polyGeofenceList = <geofence.PolyGeofence>[
    geofence.PolyGeofence(
      id: 'park1',
      data: {
        'about': 'Park #1',
      },
  ...
  geofence.Location _currentLocation =
      new geofence.Location.fromJson({'latitude': 0.0, 'longitude': 0.0});

  // This function is to be called when the geofence status is changed.
  Future<void> _onPolyGeofenceStatusChanged(
      geofence.PolyGeofence polyGeofence,
      geofence.PolyGeofenceStatus polyGeofenceStatus,
      geofence.Location location) async {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: geofence.WillStartForegroundTask(
        onWillStart: () async {
  ...
  Widget _buildContentView() {
    return StreamBuilder<geofence.PolyGeofence>(
      ...
      builder: (context, snapshot) {
        ...
        if (content != '') {
          if (snapshot.data?.status == geofence.PolyGeofenceStatus.DWELL) {
            prevParkedAt = parkedAt;
            parkedAt = 'Curr: ' + content;
          } else if (snapshot.data?.status ==
              geofence.PolyGeofenceStatus.ENTER) {
            prevEntered = entered;
            entered = 'Entry: ' + content;
          } else if (snapshot.data?.status ==
              geofence.PolyGeofenceStatus.EXIT) {
            prevLeft = left;
            left = 'Left: ' + content;
          }
        }
